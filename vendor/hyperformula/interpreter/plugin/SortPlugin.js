 ...
 * engine.
 */
export class SortPlugin extends FunctionPlugin {
    /**
     ...
     */
    sort(ast, state) {
        return this.runFunction(ast.args, state, this.metadata('SORT'), (range, sortIndex, sortOrder, byCol) => {
            ...
            if (byCol) {
                ...
                order.sort((c1, c2) => compare(data[keyIndex][c1], data[keyIndex][c2]));
                const result = data.map(row => order.map(c => row[c]));
            ...
            rows.sort((r1, r2) => compare(r1[keyIndex], r2[keyIndex]));
...
SortPlugin.implementedFunctions = {
    'SORT': {
        method: 'sort',
