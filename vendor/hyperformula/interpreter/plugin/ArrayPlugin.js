...
export class ArrayPlugin extends FunctionPlugin {
     ...
     */
    vstack(ast, state) {
        return this.runFunction(ast.args, state, this.metadata('VSTACK'), (...ranges) => {
     ...
     */
    hstack(ast, state) {
        return this.runFunction(ast.args, state, this.metadata('HSTACK'), (...ranges) => {
            const height = Math.max(...ranges.map(range => range.height()));
            const result = [...Array(height).keys()].map(() => []);
            for (const range of ranges) {
                const data = range.data;
     ...
     */
    stackSubChecks(ast, state, functionName) {
        const metadata = this.metadata(functionName);
        return ast.args.map((arg) => { var _a; return this.arraySizeForAst(arg, new InterpreterState(state.formulaAddress, state.arraysFlag || ((_a = metadata === null || metadata === void 0 ? void 0 : metadata.enableArrayArithmeticForArguments) !== null && _a !== void 0 ? _a : false))); });
    }
    /**
